...
function CFODashboardPage() {
  ...
  const createPDFDocument = (title) => {
    ...
    const addRow = (label, value) => {
      ...
      if (labelLines.length === 1 && valueLines.length === 1) {
    ...
    const addFooter = () => {
      ...
      doc.text("Generated by Financial System", margin, 285);
    };
  ...
  const generateIncomePDF = (income) => {
    const { doc, addRow, addNotes, addFooter, save } =
      createPDFDocument("Income Receipt");

    addRow("Amount", `$${income.amount.toFixed(2)}`);
    addRow("Description", income.description);
    addRow("Date", new Date(income.date).toLocaleDateString());
    addRow("Project", income.projectId?.name || "General");

    if (income.notes) {
      addNotes(income.notes);
    }

    addFooter();
    save(`income-${income._id}.pdf`);
  };

  const generateExpensePDF = (expense) => {
    const { doc, addRow, addNotes, addFooter, save } =
      createPDFDocument("Expense Report");

    addRow("Amount", `$${expense.amount.toFixed(2)}`);
    addRow("Description", expense.description);
    addRow("Date", new Date(expense.date).toLocaleDateString());
    addRow("Project", expense.projectId?.name || "General");

    if (expense.notes) {
      addNotes(expense.notes);
    }

    addFooter();
    save(`expense-${expense._id}.pdf`);
  };
